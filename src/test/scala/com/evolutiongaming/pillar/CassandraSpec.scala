 ...
 *
 * When tests are run with this mixed in, it will attempt to start an embedded instance of
 * Cassandra on a random port, which will then be usable by the unit test code.
 *
