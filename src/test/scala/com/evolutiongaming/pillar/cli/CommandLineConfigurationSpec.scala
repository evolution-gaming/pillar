  ...
  describe(".buildFromArguments") {
    describe("command initialize") {
      describe("data-store faker") {
        ...
        it("sets the command") {
          CommandLineConfiguration.buildFromArguments(
            Array("initialize", "faker"),
          ).command should be(Initialize)
          CommandLineConfiguration.buildFromArguments(
            Array("migrate", "faker"),
          ).command should be(Migrate)
        }

        it("sets the data store name") {
          CommandLineConfiguration.buildFromArguments(
            Array("initialize", "faker"),
          ).dataStore should equal("faker")
          CommandLineConfiguration.buildFromArguments(
            Array("migrate", "faker"),
          ).dataStore should equal("faker")
        }
        ...
        it("sets the environment") {
          CommandLineConfiguration.buildFromArguments(
            Array("initialize", "faker"),
          ).environment should equal("development")
          CommandLineConfiguration.buildFromArguments(
            Array("migrate", "faker"),
          ).environment should equal("development")
        }
        ...
        it("sets the time stamp") {
          CommandLineConfiguration.buildFromArguments(
            Array("initialize", "faker"),
          ).timeStampOption should be(None)
          CommandLineConfiguration.buildFromArguments(
            Array("migrate", "faker"),
          ).timeStampOption should be(None)
        }
        ...
        describe("environment test") {
          it("sets the environment") {
            CommandLineConfiguration.buildFromArguments(Array(
              "-e",
              "test",
              "initialize",
              "faker",
        ...
        describe("migrations-directory baz") {
          it("sets the migrations directory") {
            CommandLineConfiguration.buildFromArguments(Array(
              "-d",
              "src/test/resources/pillar/migrations",
              "initialize",
              "faker",
            )).migrationsDirectory
        ...
        describe("time-stamp 1370028262") {
          it("sets the time stamp option") {
            CommandLineConfiguration.buildFromArguments(Array(
              ...
              "initialize",
              "faker",
            )).timeStampOption should equal(Some(1370028262))
            CommandLineConfiguration.buildFromArguments(Array(
              "-t",
              "1370028262",
              "migrate",
