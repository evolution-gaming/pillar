    ...
    describe("without a down parameter") {
      it("returns an irreversible migration") {
        Migration.apply(
          ...
          Instant.now,
          Seq("up"),
        ).getClass should be(classOf[IrreversibleMigration])
      }
    }
      ...
      describe("when the down is None") {
        it("returns a reversible migration with no-op down") {
          Migration.apply(
            ...
            Seq("up"),
            None,
          ).getClass should be(classOf[ReversibleMigrationWithNoOpDown])
        }
      }
      ...
      describe("when the down is Some") {
        it("returns a reversible migration with no-op down") {
          Migration.apply(
            ...
            Seq("up"),
            Some(Seq("down")),
          ).getClass should be(classOf[ReversibleMigration])
        }
      }
    }
  }
