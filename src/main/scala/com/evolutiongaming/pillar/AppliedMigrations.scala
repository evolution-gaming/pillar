...
object AppliedMigrations {
  def apply(session: Session, registry: Registry, appliedMigrationsTableName: String): AppliedMigrations = {
    val results = session.execute(
      QueryBuilder.select("authored_at", "description").from(appliedMigrationsTableName),
    )
...
class AppliedMigrations(applied: Seq[Migration]) {
  ...
  def authoredAfter(date: Instant): Seq[Migration] =
    applied.filter(migration => migration.authoredAfter(date))
