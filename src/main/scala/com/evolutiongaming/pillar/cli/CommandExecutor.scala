...
object CommandExecutor {
  private implicit val migratorConstructor: ((Registry, Reporter, String) => Migrator) = Migrator.apply
...
class CommandExecutor(
  implicit
  val migratorConstructor: (Registry, Reporter, String) => Migrator,
) {
  def execute(command: Command, reporter: Reporter): Unit = {
    ...
    command.action match {
      case Initialize =>
        ...
        ()
      case Migrate =>
        migrator.migrate(command.session, command.timeStampOption.map(Instant.ofEpochMilli))
    }
  }
}
