...
object CommandLineConfiguration {
  ...
  case class ParserSuccess(
    command: Option[MigratorAction],
    dataStore: Option[String],
    environment: Option[String],
    migrationsDirectory: Option[File],
  ...
  @tailrec
  private def parseArgsRecusively(
    remainingArgs: List[String],
    parserSuccess: ParserSuccess = ParserSuccess(None, None, None, None, None),
  ): ParserResult = {
    ...
    remainingArgs match {
      ...
      case "initialize" :: tail if parserSuccess.command.isEmpty && tail.nonEmpty =>
        parseArgsRecusively(
          tail.tail,
          parserSuccess.copy(command = Some(Initialize), dataStore = Some(tail.head)),
        )

      case "migrate" :: tail if parserSuccess.command.isEmpty && tail.nonEmpty =>
        parseArgsRecusively(
          tail.tail,
          parserSuccess.copy(command = Some(Migrate), dataStore = Some(tail.head)),
        )
      ...
      case unknown =>
        ParserFailure(
          s"Failed to parse the command line - cannot handle arguments: ${ unknown.mkString(" ") }",
        )
    }
  }

  def buildFromArguments(arguments: Array[String]): CommandLineConfiguration =
    parseArgsRecusively(arguments.toList) match {
