...
object ReplicationStrategyBuilder {
   ...
   *
   * @param configuration
   *   The applications Typesafe config
   * @param dataStoreName
   *   The target data store, as defined in application.conf
   * @param environment
   *   The environment, as defined in application.conf (i.e.
   *   "pillar.dataStoreName.environment {...})
   * @return
   *   ReplicationOptions with a default of Simple Strategy with a replication factor of 3.
   */
  def getReplicationStrategy(configuration: Config, dataStoreName: String, environment: String)
    : ReplicationStrategy = try {

    val repStrategyStr =
      Try(configuration.getString(s"pillar.$dataStoreName.$environment.replicationStrategy"))
    ...
    case e: IllegalArgumentException =>
      throw new BadValue(s"pillar.$dataStoreName.$environment", e.getMessage)
